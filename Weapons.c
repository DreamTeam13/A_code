...
while(1 == 1) //Repeat code forever
 ...
 else if(joy1Btn(7) == 1) //When the left bumper is pushed, move motor A backwards
 {
 motor(motorA) = 100;
 }
 else
 ...
 motor(motorA) =0; // grabber 1
  ...
  if(joy1Btn(6) == 1) //When the right trigger is pushed, move motor A forward
 {
 motor(motorB) = -100;
 }
